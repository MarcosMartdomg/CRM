...
@Composable
fun pantallaPerfilFooter(modifier: Modifier = Modifier, navHostController: NavHostController) {
    Box(
        modifier = modifier
            .fillMaxWidth()
            .height(300.dp) // Altura del footer
            .padding(top = 140.dp)
    ) {
        val footerImage: Painter = painterResource(id = R.drawable.header_image) // Usa la misma imagen del header
        Image(
            painter = footerImage,
        ...
        Row(
            modifier = Modifier
                .fillMaxWidth()
                .align(Alignment.BottomCenter) // Alineamos las imágenes al fondo del Box
